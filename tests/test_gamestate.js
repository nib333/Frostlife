...
g = G.createGame(4, 40);
G.applyAction(g, { type: "rename", player: 0, name: "Alice" });
...
ok(g2.players[0].name === "Alice", "round-trip: name");
...
ok(ng.players[0].name === "Alice", "name carries seat-for-seat");
...
ok(rec.players.join(",") === "Alice,Eva,Player 3", "all player names listed");
ok(rec.winner === "Alice", "winner recorded by name, not index");
...
ok(g.players.length === 3 && g.players[0].name === "Alice" && !("winner" in g),
